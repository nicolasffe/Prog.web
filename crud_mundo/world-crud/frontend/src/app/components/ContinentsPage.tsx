  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ color: '#0f172a', fontSize: '1.4rem', fontWeight: 700 }}>Continents</h1>
          <p style={{ color: '#64748b', fontSize: '0.85rem', marginTop: 2 }}>{continents.length} continents in database</p>
        </div>
      ...
      <div className="relative mb-5 max-w-sm">
        ...
        <input
          ...
          style={{ background: 'white', border: '1px solid #e2e8f0', color: '#1e293b', fontSize: '0.875rem' }}
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
              {['#', 'Name', 'Code', 'Countries', 'Description', 'Actions'].map(h => (
                <th key={h} style={{ padding: '12px 16px', textAlign: 'left', color: '#64748b', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', background: '#f8fafc' }}>
                  {h}
                </th>
              ))}
            </tr>
                ...
                <tr key={c.id}
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <span style={{ color: '#1e293b', fontWeight: 600, fontSize: '0.875rem' }}>{c.name}</span>
                    </div>
                  </td>
                  <td style={{ padding: '14px 16px' }}>
                  ...
                  <td style={{ padding: '14px 16px', color: '#475569', fontSize: '0.875rem' }}>{cCount}</td>
                  <td style={{ padding: '14px 16px', color: '#64748b', fontSize: '0.82rem', maxWidth: 300 }}>
      ...
      <Modal open={modal === 'create' || modal === 'edit'} onClose={() => setModal(null)}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Field label="Description">
            <Textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Brief description of the continent..." />
          </Field>
          <Field label="Color">
          ...
          return (
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 style={{ color: '#0f172a', fontWeight: 700, fontSize: '1.2rem' }}>{editing.name}</h2>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b', fontWeight: 700, fontSize: '1.2rem', marginTop: 2 }}>{cCount}</p>
                </div>
                <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              {editing.description && (
                <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#475569', fontSize: '0.85rem', lineHeight: 1.5 }}>{editing.description}</p>
                </div>
              )}
              <div className="flex gap-2 justify-end pt-2" style={{ borderTop: '1px solid #e2e8f0' }}>
                <button onClick={() => setModal(null)} className="rounded-xl px-4 py-2 text-sm" style={{ background: '#f1f5f9', color: '#475569', border: 'none', cursor: 'pointer' }}>Close</button>
