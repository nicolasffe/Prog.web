    ...
    <div className="p-6 max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span style={{ color: '#64748b', fontSize: '0.85rem' }}>{country.isoCode} · {country.timezone}</span>
              </div>
      ...
      {editing ? (
        <form id="countryForm" onSubmit={handleSave} className="rounded-2xl p-6 mb-6 space-y-4"
          ...
          <h3 style={{ color: '#0f172a', fontWeight: 700, marginBottom: 4 }}>Edit Country Details</h3>
          ...
          <div className="grid grid-cols-3 gap-4">
            <Field label="Population"><Input type="number" value={form.population} onChange={e => setForm(f => ({ ...f!, population: Number(e.target.value) }))} /></Field>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={card.label} className="rounded-2xl p-5" style={{ background: 'white', border: '1px solid #e2e8f0' }}>
              ...
              <p style={{ color: '#1e293b', fontSize: '1rem', fontWeight: 700, marginTop: 4 }}>{card.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {(country.region || country.subregion || country.timezones?.length || country.maps?.googleMaps || country.maps?.openStreetMaps) && (
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'white', border: '1px solid #e2e8f0' }}>
          <div className="flex items-center justify-between mb-4">
            <p style={{ color: '#1e293b', fontWeight: 600 }}>External Country Data</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              <p style={{ color: '#1e293b', fontWeight: 700, marginTop: 4 }}>{[country.region, country.subregion].filter(Boolean).join(' · ') || 'N/A'}</p>
            </div>
            <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              <p style={{ color: '#94a3b8', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Timezones</p>
              <p style={{ color: '#1e293b', fontWeight: 700, marginTop: 4 }}>{country.timezones?.slice(0, 3).join(', ') || country.timezone}</p>
            </div>
            <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'white', border: '1px solid #e2e8f0' }}>
          <div className="p-5 pb-0">
            <div className="flex items-center justify-between mb-4">
              <p style={{ color: '#1e293b', fontWeight: 600 }}>Location & Coordinates</p>
          ...
          <div className="px-5 pb-5 grid grid-cols-2 gap-3">
            <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              <p style={{ color: '#1e293b', fontWeight: 700, fontFamily: 'monospace', marginTop: 2 }}>{country.lat.toFixed(4)}°N</p>
            </div>
            <div className="rounded-xl p-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              <p style={{ color: '#94a3b8', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Longitude</p>
              <p style={{ color: '#1e293b', fontWeight: 700, fontFamily: 'monospace', marginTop: 2 }}>{country.lng.toFixed(4)}°E</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {weather && (
            <div className="rounded-2xl p-5" style={{ background: 'white', border: '1px solid #e2e8f0' }}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p style={{ color: '#0f172a', fontSize: '1.8rem', fontWeight: 800 }}>{weather.temp}°C</p>
                  <p style={{ color: '#64748b', fontSize: '0.85rem' }}>{weather.condition}</p>
                </div>
                <div className="ml-auto space-y-1">
                  <div className="flex items-center gap-2">
                    <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>💧 Humidity</span>
                    <span style={{ color: '#475569', fontSize: '0.8rem', fontWeight: 600 }}>{weather.humidity}%</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Wind size={12} style={{ color: '#94a3b8' }} />
                    <span style={{ color: '#475569', fontSize: '0.8rem', fontWeight: 600 }}>{weather.wind} km/h</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-2xl p-5" style={{ background: 'white', border: '1px solid #e2e8f0' }}>
            <div className="flex items-center justify-between mb-4">
              <p style={{ color: '#1e293b', fontWeight: 600 }}>Cities ({countryCities.length})</p>
                ...
                {countryCities.map(city => (
                  <button key={city.id} onClick={() => navigate(`/app/cities/${city.id}`)}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span style={{ color: '#1e293b', fontWeight: 600, fontSize: '0.875rem' }}>{city.name}</span>
